<template>
		...
		<div style="border-top:solid rgb(8,50,150) 1px;border-left:solid rgb(8,50,150) 1px;border-right:solid rgb(8,50,150) 2px;border-bottom:solid rgb(8,50,150) 2px;box-shadow: 1px 2px 3px #888888;">
			 ...
			 <nav class="panel column is-12" style="overflow-x: auto;">
				...
				<div class="panel-block" style="padding:0px;">
				    ...
				    <p class='navbar-end pull-right'>
        			  ...
        			  <p @click="openmodal('po_add','')" class="control" title="Add New Data">
                        <a class="button is-info is-rounded is-small" style="width:170px;">
                            <i class="bi bi-plus" style="font-size:38px;"></i> <b>ADD TRANSACTION</b>
                        </a>
                      </p>
			...
			<div style="width:100%;overflow-x:auto;overflow-y:auto;">
					...
					<table class="table is-hoverable" border="2" style="width:100%;">
						...
						<tr v-else v-for="list,key in alldata" :style="list.status == 'unused' ? '' : 'background-color:rgb(255,240,240);'">
						  	...
						  	<td style="text-align:center;padding:5px;border:solid black 1px;">
								...
								<td style="padding-left:5px;border:solid black 1px;font-size:13px;">
									...
									Transaction Code: <b style="font-size:15px;color:gray;">{{ list.transaction_code }}</b>
									<br>
									...
									Cost: <b style="font-size:19px;">₱ {{ parseInt(list.suppliers_price).toFixed(2).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") }}</b> 

									&nbsp;&nbsp; 

									Total Cost: <b style="font-size:19px;">₱ {{ parseInt(list.total).toFixed(2).toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",") }}</b> 

									&nbsp;&nbsp; 

									Payment Term: <b style="font-size:18px;">{{ list.payment_terms }}</b>
								...
								<td style="border:solid black 1px;padding:5px;font-size:13px;">
									...
									Company: <b style="font-size:18px;">{{ list.company }}</b>

									<br>

									Supplier: <b style="font-size:18px;">{{ list.supplier_name }}</b>

									<br>

									Issued By: <b style="font-size:18px;">{{ list.user_name }}</b>
  ...
  <div v-if="searchModal"  id="modal-ter" class="modal is-active is-pulled-left" style="width:27%;">
	      		...
	      		<div>
			        ...
			        <div class="field">
			          <label>Transaction Code
	      				 <button class="delete is-pulled-right" aria-label="close" @click="transaction_code = ''"></button>
			          </label>
			          <div class="control">
							<input v-model='transaction_code' placeholder="Type Here.." class="input" type="text">
			          </div>
			        </div>
			        ...
			        <div class="field">
			          <label>Issued By
	      				 <button class="delete is-pulled-right" aria-label="close" @click="user_name = ''"></button>
			          </label>
			          <div class="control">
							<input v-model='user_name' placeholder="Type Here.." class="input" type="text">
			          </div>
			        </div>
			        ...
			        <div class="field">
			          <label>Total
	      				 <button class="delete is-pulled-right" aria-label="close" @click="total = ''"></button>
			          </label>
			          <div class="control">
							<input v-model='total' placeholder="Type Here.." class="input" type="number">
			          </div>
			        </div>
			        ...
			        <div class="field" style="width:100%;">
			          <label>Payment Term
	      				 <button class="delete is-pulled-right" aria-label="close" @click="payment_terms = ''"></button>
			            </label>
				            <div class="control" style="width:100%;">
				              <div class="select" style="width:100%;">

			                	<select v-model='payment_terms' style="width:100%;">
	...
	<!-- modal start -->
	  ...
	  <div v-if="selectSupplier" id="modal-ter" class="modal is-active">
		  <div class="modal-background"></div>
		  <div class="modal-card" style="width:60%;">
	        <header class="modal-head" style="background:rgb(0, 53, 170);padding:5px;">

	    	    	<b style="color:white;font-size:20px;">Select Supplier</b>

	    	    	<a class="button is-danger is-small" style="float:right;" @click="selectSupplier = false">
	    	            <i class="bi bi-x" style="font-size:38px;"></i> Close
	    	        </a>
				 		...
				 		<section class="modal-card-body" style="padding:15px;">
				      		<div>
									...
									<selectSupplier @transactioncomplete='openAdd'>
									</selectSupplier>
...
<script type="text/javascript">
    ...
    import ControlModal from '../../Modals/ItemsModal/POModal.vue';
    import selectSupplier from '../../Tables/ItemTables/selectSupplier.vue';
			            		...
			            		axios.post('/deletepo',{
				 				...
				 				.then((response) => {
									  ...
									  if (response.data == 'bad') {
									  	  ...
									  	  this.showToast({
						                    type: 'error',
						                    message: 'Error! The Item Stock is used already.',
						                  });
